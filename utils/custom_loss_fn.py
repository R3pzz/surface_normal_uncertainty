...
def vMF_masked_loss(pred, gt_norm, gt_mask):
  ...
  gt_bg_norm = gt_norm[gt_mask] # random noise sampled in the data processing stage
  gt_foot_norm = gt_norm[~gt_mask] # ground truth foot normal map

  # loss fn:
  # foot_kappa * acos(foot_norm * gt_foot_norm) + log((1 + exp(-foot_kappa * PI)) / (1 + square(foot_kappa)))
  foot_dev = torch.sum(foot_norm * gt_foot_norm, dim=1)
  foot_dev = torch.clamp(foot_dev, min=-1.0, max=1.0)
  ...
  bg_dev = torch.sum(bg_norm * gt_bg_norm, dim=1)
  bg_dev = torch.clamp(bg_dev, min=-1.0, max=1.0)
  bg_loss = torch.mean(
